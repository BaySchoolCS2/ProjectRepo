...
def profile(name = None, posts = ''):
    try:
        posts = collections.Posts.objects(author=collections.User.objects(alias=name)[0])#returns a list of posts by the first user with the alias in the variable name
    except IndexError:
        pass
